...
| Collections of structured `object` fields produce a warning                       | Use `nested` if sibling field correlation must be preserved.             |
...
cardinality. Effective cardinality is a collection only when the field is listed in
`collection_fields`, including for fields mapped as `nested`. This is checked for every adjacent minor version pair
...
| Add or remove object `properties` | No                       | Yes                         |
