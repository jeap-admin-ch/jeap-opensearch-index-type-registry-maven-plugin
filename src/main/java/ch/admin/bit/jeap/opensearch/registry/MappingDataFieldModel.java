...
public record MappingDataFieldModel(Set<String> declaredCollectionFields,
    ...
    private static void collectFields(JsonNode properties, String parentPath, Set<String> collectionFields,
        ...
        properties.properties().forEach(entry -> {
            ...
            boolean collection = collectionFields.contains(path);
            boolean structured = ("object".equals(type) || "nested".equals(type))
                    && definition.path("properties").isObject();
            fields.put(path, new FieldContract(type, collection, structured));
    ...
    public record FieldContract(String openSearchType, boolean collection, boolean structured) {
    }
}
